...
//     setError(null);
//     try {
//       const [tagsData, iosData, usersData] = await Promise.all([
//         adminApi.getTags(),
//         adminApi.getIOPreviews(),
//         adminApi.getUsers()
//       ]);
//       setTags(tagsData);
//       setIos(iosData);
...
export const AdminDataPage = () => {
  ...
  const handleApproveDelete = async (requestId: string) => {
    ...
    try {
      ...
      setError(t('admin_data.error_approve_delete', 'Ошибка при одобрении удаления объекта'));
      setIsLoading(false);
    }
  };
  ...
  const handleRejectDelete = async (requestId: string) => {
    ...
    try {
      ...
      setError(t('admin_data.error_reject_delete', 'Ошибка при отклонении запроса'));
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="col-span-full bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h2 className="text-xl font-bold mb-4 text-gray-800">{t('admin_data.deletion_requests_title', 'Запросы на удаление ИО')}</h2>
        <div className="border border-gray-200 rounded-md max-h-96 overflow-y-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
              <tr>
                <th className="p-3 text-gray-700 font-semibold">{t('admin_data.table_io_id', 'ID объекта')}</th>
                <th className="p-3 text-gray-700 font-semibold">{t('admin_data.table_author_login', 'Логин автора запроса')}</th>
                <th className="p-3 text-gray-700 font-semibold">{t('admin_data.table_reason', 'Причина удаления')}</th>
                <th className="p-3 text-gray-700 font-semibold w-40">{t('admin_data.table_actions', 'Действия')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={req.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    <div className="flex space-x-2">
                      ...
                      >
                        {t('admin_data.approve_delete', 'Одобрить удаление')}
                      </button>
                      <button
                      ...
                      >
                        {t('admin_data.reject_delete', 'Отклонить')}
                      </button>
                    </div>
                  </td>
                </tr>
              ...
              {deletionRequests.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-4 text-center text-gray-500">
                    {t('admin_data.no_deletion_requests', 'Нет активных запросов на удаление')}
                  </td>
                </tr>
              )}
            </tbody>
