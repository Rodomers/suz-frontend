...
export const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        const agreementRes = await api.get('/agreements/me');
        if (agreementRes.data && agreementRes.data.accepted) {
          rulesAccepted = true;
        }
      } catch (err) {
        if (isAxiosError(err) && err.response?.status !== 404) {
      ...
      if (!rulesAccepted) {
        const mappedUser = mapUser(userRes.data, false);
        setAuth(mappedUser);
        navigate('/rules');
        return;
      }
  ...
  return (
    <div className="login-page-container flex justify-center items-center h-screen bg-gray-50">
      {showExpiryBanner && user?.access_end && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-amber-500 text-white p-3 text-center text-sm font-medium">
          Внимание! Срок действия вашей учетной записи ограничен до {user.access_end}. По истечении этого времени вы будете автоматически деавторизованы.
        </div>
      )}
      <style>{`
        .login-page-container {
